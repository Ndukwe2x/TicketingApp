...
export default async function Employees() {
    ...
    React.useEffect(() => {
        (async (user) => {
            ...
            const data = result.filter(row => ['owner','staff'].includes(row.accountType));
            setEmployees(data);
        })(user);
    ...
    return (
        <div className='flex flex-col gap-5'>
            <Text variant='h1'>Employees</Text>
            <Card>
                <CardContent className='pt-5'>
                    <DataTable columns={columns} data={employees} fallback={<LoadingDashboardUsers columnCount={9} />} />
                </CardContent>
            </Card>
        </div>
    );
