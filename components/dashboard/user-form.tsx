...
import { BiPencil, BiUser, BiUserPlus } from "react-icons/bi";
...
function AccountAvatar({ account }: { account: AppUser | null }) {
    ...
    const handlePickAvatar = (ev: ChangeEvent<HTMLInputElement>) => {
        ...
        parseFileToDataUri(file)
            .then((dataUri) => {
                ...
                if (!avatarPreview.classList.contains('loaded')) {
    ...
    const resetFilePicker = (ev: MouseEvent) => {
        if (!filePickerRef.current || !selectedFilePreviewRef.current) {
            return;
        }
    ...
    const actionBtns = (
        <span className="flex items-center absolute right-0 left-0 px-4 gap-4 justify-center">
            <span className={cn(editBtnClass, 'bg-black/80 text-white px-2 py-2 rounded-full ')}>
                <FaRetweet size={28} />
            </span>
    ...
    return (
        <div className="flex flex-col items-center">
            ...
            <Label className="fancy-file-picker border p-5 rounded-xl bg-gray-200 cursor-pointer relative">
                <Input type='file' accept='image/jpeg,image/png'
                    ref={filePickerRef}
                    name='user_avatar' required className="hidden" onChange={handlePickAvatar} />
                {
                    (account && account.avatar) ? (
                        <span ref={selectedFilePreviewRef} className="avatar-upload-preview flex items-center loaded" style={{ backgroundImage: account.avatar }}>
                            <NextImage src={account.avatar} alt={account.fullName} title={account.fullName} width={200} height={200} />
                            {actionBtns}
                        </span>
                    ) : (
                        <React.Fragment>
                            <BiUser size={160} className="text-muted-foreground" />
                            <span ref={selectedFilePreviewRef} className="avatar-upload-preview flex items-center">
                                {actionBtns}
                            </span>
                        </React.Fragment>
                    )
